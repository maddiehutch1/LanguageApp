...
protocol LessonPlan {
    ...
    mutating func toggleLessonRead(for topic: String);
}
...
struct Language {
    ...
    struct Term: Hashable, Equatable, Identifiable {
        let id = UUID()
        let word: String
        let translation: String
    }
    
    struct QuizItem: Hashable, Equatable, Identifiable {
        let id = UUID()
        let question: String
        let answers: [String]
...
// for the flaschard view
struct Flashcards<CardContent> {
...
// for the lesson view?
struct LessonContent<Content> {
    
}

struct QuizContent<QuestionContent> {
